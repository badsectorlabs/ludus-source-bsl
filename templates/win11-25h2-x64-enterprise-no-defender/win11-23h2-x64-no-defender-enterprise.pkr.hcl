variable "description" {
  type    = string
  default = "Windows 11 25H2 Enterprise (x64) with Windows Defender disabled."
}
...
variable "vm_name" {
  type    = string
  default = "win11-25h2-x64-enterprise-no-defender-template"
}
...
locals {
  template_description = "Windows 11 25H2 64-bit Enterprise No Defender template built ${legacy_isotime("2006-01-02 03:04:05")} username:password => localuser:password"
}

source "proxmox-iso" "win11-25h2-x64-no-defender-enterprise" {
  ...
  disks {
    ...
    type              = "scsi" # virtio here causes a "inaccessible boot device 0x7b" error, despite loading the driver. SCSI works
    discard           = true
    io_thread         = true
    ssd               = true
  }
...
build {
  sources = ["source.proxmox-iso.win11-25h2-x64-no-defender-enterprise"]
